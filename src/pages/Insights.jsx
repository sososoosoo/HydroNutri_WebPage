...
const newsMeta = [
  { key: 1, image: '/assets/news1.jpg', date: 'Oct 14, 2024', link: '#' },
  { key: 2, image: '/assets/blog-default.jpg', date: 'Mar 10, 2025', link: '#' },
  { key: 3, image: '/assets/news2.jpg', date: 'Nov 02, 2024', link: '#' },
];
...
export default function Insights() {
  ...
  return (
    <div className="bg-ultra-dark min-h-screen pb-20">
      <SubPageHero
        category="insights"
        title={t('insights.heroTitle')}
        currentPage={t('insights.heroTitle')}
      ...
      <section className="premium-section pt-24 pb-16">
        <motion.div className="max-w-4xl mx-auto px-6 text-center" initial="hidden" animate="visible" variants={fadeUp}>
           <h2 className="premium-heading mb-6">{t('insights.heading')} <span className="highlight-glow">{t('insights.headingHighlight')}</span></h2>
           <p className="premium-subtext">{t('insights.desc')}</p>
        </motion.div>
      </section>
              ...
              >
                 <div className="h-48 overflow-hidden relative">
                    ...
                    <div className="absolute top-4 left-4 z-20 px-3 py-1 rounded-full bg-cyan-500/20 border border-cyan-500/50 text-cyan-400 text-xs font-bold uppercase tracking-widest backdrop-blur-md shadow-lg">
                      {t(`insights.news${news.key}Category`)}
                    </div>
                 </div>
                 <div className="p-8 flex flex-col grow">
                    <div className="flex justify-between items-center mb-4 text-slate-500 text-sm font-mono">
                       <span>{t(`insights.news${news.key}Author`)}</span>
                       <span>{news.date}</span>
                    </div>
                    <h3 className="text-xl font-bold text-white mb-4 leading-tight group-hover:text-cyan-400 transition-colors">{t(`insights.news${news.key}Title`)}</h3>
                    <p className="text-slate-400 text-sm leading-relaxed mb-6">{t(`insights.news${news.key}Summary`)}</p>
                    <div className="mt-auto flex items-center gap-2 text-cyan-400 text-sm font-bold uppercase tracking-widest pt-6 border-t border-slate-800">
                       {t('insights.readArticle')} <ArrowUpRight size={16} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                    </div>
                 </div>
              </motion.a>
            ))}
